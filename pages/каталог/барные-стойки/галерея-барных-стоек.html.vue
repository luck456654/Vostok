...
<div class="col-xs-12 nav-content">
      <div class="crumbs">
          <ol itemscope itemtype="http://schema.org/BreadcrumbList" class="breadcrumb" :class="{breadcrumb_open: showBreadcrumbsMobile}">
            ...
            <li class="breadcrumbs-links" itemprop="itemListElement" itemscope
              itemtype="http://schema.org/ListItem">
                <nuxt-link to="/каталог/барные-стойки.html" append itemprop="item">
                  <span itemprop="name">Барные стойки</span>
                </nuxt-link>
...
<a class="zakaz-kamen content-zakaz" @click.prevent="showModalCallBack = !showModalCallBack">Заказать барную стойку</a>
  ...
  <div class="col-sm-12 i-tab owl-paltra_wrapper exampels_photo">
    <div class="row">
      <div class="catalog-preview_block col-sm-4" v-for="(item, index) in arr" :key="index"  >
        <a
          data-fancybox
          :href="'https://стильный-камень.рф/assets/images/resources/619/'+item.img"
          class="catalog-preview_wrapper"
          :style="{'background-image':'url(https://стильный-камень.рф/assets/images/resources/619/360x270/'+item.url+')'}"
          exact
        ...
        <div v-if="openSuperModal == index+1"  class="modal-stone" >
          ...
          <div class="row modal-stone-row">
            ...
            <div class="modal-stone-img-block"><img :src="'https://стильный-камень.рф/assets/images/resources/619/'+item.img" class="modal-stone-img"></div>
...
export default {
  head() {
    return{
      title:"Фото барных стоек из камня",
      meta: [
        { name: 'description', hid: 'description', content: 'Посмотрите галерею изготовленных и установленных барных стоек из камня.'}
      ],
      link: [
      { rel: 'canonical', href: 'https://стильный-камень.рф/каталог/барные-стойки/галерея-барных-стоек.html'},
      ]
    };
  },
    components: {
    ...
    data() {
        return {
          page_name:"Галерея барных стоек",
     ...
     async asyncData({
        ...
        return await app.$axios
            .$get(
                `https://стильный-камень.рф/data/katalog/data_katalog.php?data=gallery_bar`
            )
            .then(arr => {
                return {
                    arr:arr,
