...
<div class="col-xs-12 nav-content">
          ...
          <ol itemscope itemtype="http://schema.org/BreadcrumbList" class="breadcrumb" :class="{breadcrumb_open: showBreadcrumbsMobile}">
            ...
            <li class="breadcrumbs-links active" itemprop="itemListElement" itemscope
              itemtype="http://schema.org/ListItem">
                <nuxt-link to="../мойки-из-акрилового-камня-для-ванной/"  itemprop="item">
                  <span itemprop="name">Мойки из акрилового камня для ванной</span>
                 </nuxt-link>
...
export default {
   ...
   components: {
    ...
    data() {
        return {
          up_link:"../мойки-из-акрилового-камня-для-ванной/",
    ...
    async asyncData({
        ...
        return await app.$axios
            .$get(
                `https://стильный-камень.рф/data/katalog/data_moyki.php?data=akril_bath&url=${encodeURIComponent(uriPage)}`
            )
            .then((response) => {
                return {
