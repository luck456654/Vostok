...
<div class="col-xs-12 nav-content">
          ...
          <ol itemscope itemtype="http://schema.org/BreadcrumbList" class="breadcrumb" :class="{breadcrumb_open: showBreadcrumbsMobile}">
            ...
            <li class="breadcrumbs-links active" itemprop="itemListElement" itemscope
              itemtype="http://schema.org/ListItem">
                <nuxt-link to="../мойки-из-кварца.html"  itemprop="item">
                  <span itemprop="name">Кухонные мойки из кварца</span>
                 </nuxt-link>
   ...
   components: {
    ...
    async asyncData({
        ...
        return await app.$axios
            .$get(
                `https://стильный-камень.рф/data/katalog/data_moyki.php?data=kvartz&url=${encodeURIComponent(uriPage)}`
            )
            .then((response) => {
                return {
