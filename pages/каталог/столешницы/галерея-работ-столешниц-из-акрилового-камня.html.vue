  ...
  <div class="col-sm-12 i-tab owl-paltra_wrapper exampels_photo">
    <div class="row">
      <div class="catalog-preview_block col-sm-4" v-for="(item, index) in arr" :key="index"  >
        <a
          data-fancybox
          :href="'https://стильный-камень.рф/assets/images/resources/2612/'+item.img"
          class="catalog-preview_wrapper"
          :style="{'background-image':'url(https://стильный-камень.рф/assets/images/resources/2612/360x270/'+item.url+')'}"
          exact
        ...
        <div v-if="openSuperModal == index+1"  class="modal-stone" >
          ...
          <div class="row modal-stone-row">
            ...
            <div class="modal-stone-img-block"><img :src="'https://стильный-камень.рф/assets/images/resources/2612/'+item.img" class="modal-stone-img"></div>
...
export default {
  head() {
    return{
      title:"Фото столешниц из акрилового камня для кухни и ванной",
      meta: [
        { name: 'description', hid: 'description', content: 'Каталог работ установленных столешниц из камня в Москве, Рязани и Владимире.'}
      ],
      link: [
      { rel: 'canonical', href: 'https://стильный-камень.рф/каталог/столешницы/галерея-работ-столешниц-из-акрилового-камня.html'},
      ]
    };
  },
    components: {
    ...
    data() {
        return {
          page_name:"Галерея работ столешниц из акрилового камня",
     ...
     async asyncData({
        ...
        return await app.$axios
            .$get(
                `https://стильный-камень.рф/data/katalog/data_katalog.php?data=gallery_st_acril`
            )
            .then(arr => {
                return {
                    arr:arr,
