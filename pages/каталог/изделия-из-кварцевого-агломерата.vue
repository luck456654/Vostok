          ...
          <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 catalog-preview_block"  v-for="(item, index) in content_arr" :key="index">
              <div class="img" v-if="item.old">
                  <a :href="item.old" >
                    <picture>
                        <source type="image/webp" :srcset="'https://стильный-камень.рф/pictures/category/kvartz/'+item.img+'.webp'" class="hover-effect-2" :alt="item.name">
                        <source type="image/jpg" :srcset="'https://стильный-камень.рф/pictures/category/kvartz/'+item.img+'.jpg'" class="hover-effect-2" :alt="item.name">
                        <img :src="'https://стильный-камень.рф/pictures/category/kvartz/'+item.img+'.jpg'" class="hover-effect-2" alt="Кварцевые столешницы для кухни на заказ">
                      </picture>
...
export default {
    ...
    return {
      ...
      link: [
      { rel: 'canonical', href: 'https://стильный-камень.рф/каталог/изделия-из-кварцевого-агломерата.html'},
      ],
      title: this.title
    };
  },
  ...
  data() {
    ...
    async asyncData({
        ...
        return await app.$axios
            .$get(
                `https://стильный-камень.рф/data/katalog/data_katalog.php?data=kvartz`
            )
            .then(arr => {
                return {
