...
<form action="" method="post" id="contacts_form" name="contacts_form" class="ajax_form incontent-form" v-if="currentCBP == 0">
    ...
    <div class="form-groop" style="min-height: 50px">
      ...
      <button class="content-zakaz send-button send-fade" id="send" @click.prevent="checkForm()" v-else >Отправить</button>
    </div>
...
<style scoped>
/*input::placeholder{color:#fc7f01}*/
.form-alert {
    box-shadow: 0px 0px 10px 0px red inset!important;
}
.send-fade {
  color: #9f4000!important;
  background-color: #e6e6e6;
  border: 1px solid #9f4000;
  box-shadow: 0px 2px 7px 0px #9f4000;
;
    margin-bottom: 20px;
}
.send-button{
  ...
  data() {
   ...
   mounted() {
      const scene3 = this.$scrollmagic.scene({
        triggerElement: '#contacts_form .input',
        triggerHook: 0.7,
        duration: '100%'
      })
        .setTween('input', {
          css: {
            border: '1px solid rgb(242, 110, 1)',
            boxShadow: '0px 0px 15px 0px #f90 inset',
            }
      })
