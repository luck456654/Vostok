<template>
      <section class="calc-section">
        <h1 v-if="typeH=='h1'">{{title}}</h1>
        <h3 v-else-if="typeH=='h3'">{{title}}</h3>
  ...
  data() {
    return {
      arrCalc:[
        {
          id:'01',
          name:'Выберите тип недвижимости',
          type:'selectBtn',
          desc:'Наши мастера выполняют комплексный ремонт под ключ в новостройках или частичную отделку помещений – спектр работ согласовывается напрямую с заказчиком и может меняться в зависимости от пожеланий клиента.',
          child:[
            {name:'Новостройка' ,active:true},
            {name:'Вторичное жилье' ,active:false},
            {name:'Загородный дом' ,active:false},
          ]
        },
        {
          id:'02',
          name:'Выберите тип ремонта',
          type:'selectBtn',
          desc:'Результат капитального ремонта – обновленное пространство для жильцов. Мы проводим комплексную отделку под ключ и частичный ремонт квартир в Москве и Московской области – в общем, мы решаем любые задачи по желанию заказчиков.',
          child:[
            {name:'Черновой' ,active:true},
            {name:'Косметический' ,active:false},
            {name:'Капитальный' ,active:false},
          ]
        },
        {
          id:'03',
          name:'Нужен ли демонтаж?',
          type:'selectBtn',
          desc:'Внимание, демонтаж нужен только при ремонте квартир во вторичном жилье',
          child:[
            {name:'Да' ,active:true},
        ...
        {
          id:'04',
          name:'Нужен ли выравнивать стены?',
          type:'selectBtn',
          desc:'Даже ровные стены имеют небольшие перепады, и для того, чтобы отделка оставалась в сохранности долгие годы, необходимо провести качественные штукатурные работ',
          child:[
            {name:'Да' ,active:true},
            {name:'Нет' ,active:false}
          ]
        ...
        {
          id:'05',
          name:'Нужна ли перепланировка?',
          type:'selectBtn',
          desc:'Мы предложим варианты оптимальной организации полезной площади и увеличения пространства вашего жилья без перепланировки - так, чтобы она стала более комфортной для проживания.',
          child:[
            {name:'Да' ,active:true},
            {name:'Нет' ,active:false}
          ]
      ...
      arrCalcValue:{
        homeType:'Новостройка',
        renovationType:'Черновой',
        dismantling:'Да',
        alignment:'Да',
        replanning:'Да',
        dismantling2:'Да',
    ...
    changeBtn(val1,val2){
      switch (val1) {
        case 0:
          ...
          this.arrCalcValue.homeType = this.arrCalc[val1].child[val2].name;
        break;
        case 1:
          for (let i = 0; i < this.arrCalc[val1].child.length; ++i) {
            this.arrCalc[val1].child[i].active = false;
          ...
          this.arrCalc[val1].child[val2].active = true;
          this.arrCalcValue.renovationType = this.arrCalc[val1].child[val2].name;
        break;
        case 2:
          for (let i = 0; i < this.arrCalc[val1].child.length; ++i) {
            this.arrCalc[val1].child[i].active = false;
          ...
          this.arrCalc[val1].child[val2].active = true;
          this.arrCalcValue.dismantling = this.arrCalc[val1].child[val2].name;
        break;
        case 3:
          for (let i = 0; i < this.arrCalc[val1].child.length; ++i) {
            this.arrCalc[val1].child[i].active = false;
          ...
          this.arrCalc[val1].child[val2].active = true;
          this.arrCalcValue.alignment = this.arrCalc[val1].child[val2].name;
        break;
        case 4:
          for (let i = 0; i < this.arrCalc[val1].child.length; ++i) {
            this.arrCalc[val1].child[i].active = false;
          ...
          this.arrCalc[val1].child[val2].active = true;
          this.arrCalcValue.replanning = this.arrCalc[val1].child[val2].name;
        break;
        case 5:
          for (let i = 0; i < this.arrCalc[val1].child.length; ++i) {
            this.arrCalc[val1].child[i].active = false;
          ...
          this.arrCalc[val1].child[val2].active = true;
          this.arrCalcValue.rooms = this.arrCalc[val1].child[val2].name;
        break;
        default:
        break;
      }
    ...
    async sendform(){
      if(this.form.email){
        ...
        data: {
          ...
          homeType: this.arrCalcValue.homeType,
          renovationType: this.arrCalcValue.renovationType,
          dismantling: this.arrCalcValue.dismantling,
          alignment: this.arrCalcValue.alignment,
          replanning: this.arrCalcValue.replanning,
          rooms: this.arrCalcValue.rooms,
          size: this.arrCalcValue.size,
          height: this.arrCalcValue.height,
        }
        })
        .then((res) => {
          if(res.code != 2) {
